package se.fusion1013.plugin.cobaltmagick.database.itemlock;
...
public class ItemLockDaoSQLite extends Dao implements IItemLockDao {
    ...
    public Map<UUID, ItemLock> getLocks() {
        ...
        try (
            ...
            while (rs.next()) {
                ...
                locks.put(uuid, new ItemLock(uuid, location, item, activatable));
            }
        } catch (SQLException ex) {
            ex.printStackTrace();
        }
    ...
    public void insertLockSync(ItemLock lock) {
        ...
        try (
            ...
            ps.setString(2, lock.getActivatable().getUuid().toString());
